-- El desglose "Por empleado" (tanto en vivo, obtener_resumen_periodo_
-- actual_por_empleado(), como el snapshot que guarda un corte,
-- generar_corte_caja()) mostraba en "comisión" lo PENDIENTE de pagar a
-- ese empleado — confundía, porque casi siempre ya se le había pagado
-- vía "Procesar corte" en Comisiones y ahí se veía $0 aunque el
-- empleado sí generó ingresos. A petición del usuario: ahora ese
-- número es lo que SÍ se le pagó (cut_id is not null) dentro del
-- periodo, no lo pendiente.
--
-- El total "comisiones_pendientes" del corte completo (calcular_
-- resumen_periodo, el pasivo del negocio) NO cambia — sigue siendo
-- pendiente, eso es correcto tal cual está.
--
-- No se toca ningún corte ya generado (caja_cut_employee_stats es un
-- snapshot histórico, nunca se sobreescribe) — este cambio solo afecta
-- el resumen en vivo y los cortes que se generen de aquí en adelante.
    ...
    select
      ...
      coalesce((
        ...
        where ce.employee_id = p.id and ce.tenant_id = v_tenant_id
          and ce.cut_id is not null and ce.earned_at > v_desde and ce.earned_at <= v_hasta
      ), 0)
